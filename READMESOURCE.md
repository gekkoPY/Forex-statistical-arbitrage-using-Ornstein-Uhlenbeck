...
[cite_start]**Primary Source:** *Optimal Mean Reversion Trading with Transaction Costs and Stop-Loss Exit* (Leung & Li, 2015)[cite: 17, 280].

**The Engineering Problem:**
Standard pairs trading models rely on simple distance-based metrics, such as Z-scores or Bollinger Bands. However, these models frequently collapse in retail environments because crossing the bid-ask spread on two separate assets completely erodes the profit margin. [cite_start]Theoretical models often ignore this friction[cite: 63].

**The Academic Application:**
To solve this, the engine completely abandons arbitrary indicators. [cite_start]Instead, it formalizes the cointegrated relationship between the two assets using the Ornstein-Uhlenbeck (OU) stochastic process[cite: 22]. The spread is modeled via the stochastic differential equation: 
[cite_start]$dX_{t}=\mu(\theta-X_{t})dt+\sigma dW_{t}$[cite: 26].

[cite_start]By treating the trade lifecycle as an optimal double stopping problem, the algorithm dynamically extracts the equilibrium mean ($\theta$), the reversion velocity ($\mu$), and the instantaneous volatility ($\sigma$)[cite: 29, 33]. [cite_start]Crucially, the Leung and Li (2015) framework ensures viability by explicitly incorporating fixed transaction costs directly into the Hamilton-Jacobi-Bellman (HJB) variational inequalities[cite: 40]. [cite_start]The algorithm mathematically guarantees that a trade is only executed when the expected reversion strictly exceeds the broker's spread[cite: 43, 44].
...
**The Academic Application:**
This codebase utilizes the findings of Lee and Leung (2020) to entirely bypass the half-life trap. [cite_start]Their research demonstrates that dynamically optimizing the exit rule—rather than passively waiting for a complete reversion to the mean—drastically improves the annualized return profile[cite: 36].

[cite_start]The algorithm's Python execution engine is programmed to calculate a dynamically bounded take-profit interval[cite: 37]. By doing so, it mathematically targets the highest-velocity phase of the snap-back, exiting the position early to maximize yield and immediately free up margin for the next execution. 
